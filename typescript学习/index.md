...
- 编译成 JS 后，大概是：
  ...
  `interface User { ... }` 整段会被删掉，运行时根本没有 `User` 这个东西，也不可能 `new User()` 或 `User instanceof xxx`。
...
```
ts
复制代码class DeleteFileOutput {
  deleted_file?: string = "default.txt";
}

const o = new DeleteFileOutput();
console.log(o.deleted_file); // "default.txt"
```
